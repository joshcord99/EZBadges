    ...
    class Util
    {
        ...
        public static void MakeCSV(List<Employee> employees)
        {
            ...
            using (StreamWriter file = new StreamWriter(filePath, append: true))
            {
                ...
                // Loop over employees and write their data
            ...
            Console.WriteLine($"Employees successfully written to {filePath}");
        }
        ...
        async public static Task MakeBadges(List<Employee> employees)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                for (int i = 0; i < employees.Count; i++)
                {
                    ...
                    // Load badge background
                    SKImage background = SKImage.FromEncodedData(File.OpenRead("badge.png"));

                    // Create a new badge canvas
                    SKBitmap badge = new SKBitmap(BADGE_WIDTH, BADGE_HEIGHT);
                    SKCanvas canvas = new SKCanvas(badge);

                    // Draw background and photo
                    ...
                    // Save the final badge image
                    SKImage finalImage = SKImage.FromBitmap(badge);
                    SKData data = finalImage.Encode();

                    // Ensure unique filename for each badge
